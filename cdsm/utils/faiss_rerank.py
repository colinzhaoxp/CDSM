...
def compute_jaccard_distance(target_features, k1=20, k2=6, print_flag=True, search_option=0, use_float16=False, lambda_value=0):
    end = time.time()
    if print_flag:
    ...
    if lambda_value > 0:
        N = target_features.size(0)
        original_dis = torch.pow(target_features, 2).sum(dim=1, keepdim=True) * 2
        original_dis = original_dis.expand(N, N) - 2 * torch.mm(target_features, target_features.t())
        original_dis /= original_dis.max(0)[0]
        jaccard_dist = jaccard_dist * (1-lambda_value) + original_dis.cpu().numpy() * lambda_value
